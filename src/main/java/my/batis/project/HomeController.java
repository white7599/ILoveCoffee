...
public class HomeController {
	...
	public String goAsk(HttpSession session, Model model, HttpServletRequest request) {	
	...
	public String loginGet(HttpServletRequest request, HttpSession session) { 
		logger.info("prevPage="+request.getHeader("referer"));
		
		String prevPage = request.getHeader("referer"); 
		if(prevPage !="http://localhost:8090/") { 
			session.setAttribute("sessionPrevPage", prevPage.replace("http://localhost:8090/", "")); 
	    }
	...
	public String loginPOST(MemberVO memberVo, HttpSession session, Model model) {
		//+request.getHeader("referer"));	// --> login http://localhost:8090/login
		String prevPage = (String) session.getAttribute("sessionPrevPage");
		logger.info("loginPOST()="+prevPage);
		String returnUrl = "redirect:/";
		if(prevPage.contains("detail?product_idx=")) {
			returnUrl = "redirect:/"+prevPage;
		}
	...
	public String goDetail(@RequestParam("product_idx") Integer product_idx, Model model, HttpServletRequest request) {
	...
	public String cartInsert(HttpSession session, CartVO vo, HttpServletRequest request) {
		String userId = (String) session.getAttribute("sessionUserID");	
